...
contract CollateralRatioGuardian is Ownable {
    ...
    */
    ...
    function setAutoRepayment(address[] memory vaults, RepaymentSetting[] memory settings) external {
        ...
        for(uint i; i < vaults.length;i++) {
            ...
            require(settings[i].triggerCollateralRatio > configurator.getBadCollateralRatio(vaults[i]), "The triggerCollateralRatio needs to be higher than Zybra.badCollateralRatio.");
            require(settings[i].expectedCollateralRatio >= configurator.getSafeCollateralRatio(vaults[i]), "The expectedCollateralRatio needs to be greater than or equal to Zybra.safeCollateralRatio");
    ...
    */
    function execute(address user, address vault) external {
        ...
        IZybra ZybraPool = IZybra(vault);
        uint256 targetDebt = (ZybraPool.depositedAsset(user) * getAssetPrice(vault)) * 100 / userSetting.expectedCollateralRatio;
        uint256 repayAmount = ZybraPool.getBorrowedOf(user) - targetDebt ;
        IERC20 token = ZybraPool.getVaultType() == 0 ? IERC20(configurator.getEUSDAddress()) : IERC20(configurator.peUSD());
        token.transferFrom(user, address(this), repayAmount + fee);
        ZybraPool.burn(user, repayAmount);
    ...
    */
    function checkExecutionFeasibility(address user, address vault) external view returns(bool) {
        ...
        IZybra ZybraPool = IZybra(vault);
        uint256 targetDebt = (ZybraPool.depositedAsset(user) * getAssetPrice(vault)) * 100 / userSetting.expectedCollateralRatio;
        uint256 totalAmount = ZybraPool.getBorrowedOf(user) - targetDebt + fee;
        IERC20 token = ZybraPool.getVaultType() == 0 ? IERC20(configurator.getEUSDAddress()) : IERC20(configurator.peUSD());
    ...
    */
    function getCollateralRatio(address user, address vault) public view returns (uint256) {
        IZybra ZybraPool = IZybra(vault);
        if (ZybraPool.getBorrowedOf(user) == 0) return 1e69;
        return
            ZybraPool.depositedAsset(user) * getAssetPrice(vault) * 100 /
    ...
    function getAssetPrice(address vault) public view returns (uint256) {
        ...
        return mulDiv(10 ** 18, price, MathLib.Rounding.Down);
    }
