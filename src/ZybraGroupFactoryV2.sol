...
import {ZybraGroupV2Refactored as ZybraGroupV2} from "./ZybraGroupV2.sol";
...
contract ZybraGroupFactoryV2 {
    ...
    function _deployGroup(
        ...
        // Deploy new ZybraGroupV2
        try new ZybraGroupV2(
            _asset,
            _contributionAmount,
            _cycleDuration,
            _totalCycles,
        ...
        returns (GroupInfo[] memory infos)
    {
        ...
        for (uint256 i = 0; i < _groups.length; i++) {
            if (isDeployedGroup[_groups[i]]) {
                ZybraGroupV2 group = ZybraGroupV2(_groups[i]);
                infos[i] = GroupInfo({
